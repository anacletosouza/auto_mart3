#!/usr/bin/env python3
"""
Generate a GROMACS topology (.top) file including selected force field,
ions, solvent, and optional ligand definitions.

This script automates the creation of a topology file by assembling the
necessary #include directives and defining the system and molecule counts.

Usage example:
    python 2-generate_cg_top.py \
        --path_ff ../ff_files/ \
...
Arguments:
    --path_ff           Directory containing force field ITP files.
    --ff                Force field ITP file (default: martini_v3.0.0.itp).
    --ions              Ions ITP file (default: martini_v3.0.0_ions_v1.itp).
    --solvent           Solvent ITP file (default: martini_v3.0.0_solvents_v1.itp).
    --itp_ligand        Ligand ITP file to include (use "None" to skip).
    --name_molecule     Name of the molecule to be listed in the topology.
    --number_molecule   Number of molecules in the system.
    --title_comments    Optional comment header for the topology file.
    --title_system      Optional system name (used in [ system ] section).
    --output_topol      Output topology (.top) filename.

Output:
    A valid GROMACS topology (.top) file ready for simulation setup.
"""
import argparse
import os
